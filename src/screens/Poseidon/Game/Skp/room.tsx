...
export const PoseidonSkpRoom: NavigationScreenComponent<any, any> = (
  props
) => {
  ...
  const gameHandler = (no: number) => {
    wsClient?.sendMessage(joinEvent, { codename: "banker-skp", no: no });
  };
  ...
  useEffect(() => {
    ...
    wsClient.sendMessage(lobbyRoomsEvent, { codename: "banker-skp" });
